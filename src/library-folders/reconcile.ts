...
function sameQuickIdentity(a: LibraryFolderSourceEntry, b: StoredLibraryFolderEntry): boolean {
...
function renameCandidate(
  ...
  const matches = missing.filter(
    (stored) =>
      stored.bookId && extension(stored.fileName) === extension(entry.fileName) && sameQuickIdentity(entry, stored),
  );
...
export function reconcileLibraryFolderScan(input: {
  ...
  for (const source of input.sourceEntries) {
    ...
    const sameContentMetadata = stored ? sameQuickIdentity(source, stored) : false;
    ...
    if (stored) {
      observedEntries.push({
        ...
        folderId: input.folder.id,
        signature: excludedStatus ? stored.signature : libraryFolderEntrySignature(source),
        status: stored.status === 'failed' ? 'failed' : 'linked',
