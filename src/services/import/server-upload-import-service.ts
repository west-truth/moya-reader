...
import { mapServerBook, mapServerReadingPosition, RemoteApiClient, RemoteApiError, RemoteUploadStatus } from '../remote/remote-api-client';
...
const importStages: ImportProgress['status'][] = ['queued', 'reading', 'decoding', 'splitting_chapters', 'writing', 'ready', 'failed'];
...
export class ServerUploadImportService implements ImportService {
  ...
  private async runImport(
    ...
    let uploadSession: { uploadId: string; sessionKey: string; status?: RemoteUploadStatus; importJobId?: string } | undefined;
    ...
    try {
      ...
      const job = uploadSession.importJobId
        ...
        : await this.uploadAndComplete(localJobId, input, uploadSession.uploadId, totalChunks, uploadSession.status, signal, onProgress);
  ...
  private async uploadAndComplete(
    ...
    for (let chunkIndex = 0; chunkIndex < totalChunks; chunkIndex += 1) {
      ...
      uploadStatus = await this.uploadChunkWithResume(localJobId, input, uploadId, chunkIndex, totalChunks, uploadStatus, signal, onProgress);
  ...
  private canResumeStoredUpload(input: ImportFileInput, totalChunks: number, uploadId: string, status: RemoteUploadStatus): boolean {
    return (
  ...
  private canResumeImportJob(input: ImportFileInput, totalChunks: number, uploadId: string, status: RemoteUploadStatus): status is RemoteUploadStatus & { importJobId: string } {
    return (
      status.status !== 'uploading' &&
      status.uploadId === uploadId &&
  ...
  private async completeUploadWithResume(
    ...
    try {
      ...
      const reconciledStatus = await this.reconcileMissingChunks(localJobId, input, uploadId, totalChunks, status, signal, onProgress);
