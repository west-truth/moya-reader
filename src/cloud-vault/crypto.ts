...
async function deriveKey(passphrase: string, salt: Uint8Array, iterations: number): Promise<CryptoKey> {
  if (passphrase.length < 12) throw new Error('Cloud vault passphrase must be at least 12 characters.');
...
export async function encryptCloudVault(snapshot: CloudVaultSnapshotV1, passphrase: string): Promise<Uint8Array> {
  ...
  const plaintext = textEncoder.encode(canonicalJson(snapshot));
...
export async function decryptCloudVault(bytes: Uint8Array, passphrase: string): Promise<CloudVaultSnapshotV1> {
  ...
  try {
    ...
    return validateSnapshot(JSON.parse(textDecoder.decode(plaintext)));
  } catch (error) {
