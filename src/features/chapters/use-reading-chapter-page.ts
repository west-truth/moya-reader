...
export function useReadingChapterPage(key: string, currentIndex: number, count: number, pageSize: number) {
  ...
  const followCurrent = !visit.current.settled && currentIndex >= 0;
  ...
  useEffect(() => {
    ...
    // Empty/loading catalogs must not erase a remembered page or consume late reading state.
    if (count > 0 && page !== remembered) remember(page);
  }, [count, followCurrent, key, page, remember, remembered]);
