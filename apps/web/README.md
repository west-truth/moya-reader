...
- Google 로그인 → 선택적 Drive 동기화 연결. 기존 암호화 독서 기록과 선택한 원본/표지를 개인 Drive에 저장한다.
  등록 정보가 없는 배포에서는 아직 연결할 수 없다. [클라이언트 ID 등록·검증 안내](../../docs/operations/google-login-drive-setup.md).
- 선택적 `VITE_GOOGLE_AUTH_URL` 설정 시 브라우저 재시작 후 로그인 복원과 Drive 토큰 자동 갱신.
  [작은 인증 서버](../../services/web-auth/README.md)만 추가하며 책 전송·저장은 브라우저와 개인 Drive에서 처리한다.
  현재 장기 연결은 코드·합성 테스트까지 완료했고 실제 계정 연결/공개 배포는 보류했다.
...
다른 기기와 이어 읽으려면 **동기화 → Google로 로그인 → Google Drive 동기화 켜기**를 사용합니다.
처음 연결할 때 동기화 암호를 정하고 다른 기기에서는 같은 암호를 사용합니다. 원본까지 필요하면 **책 파일과 표지도 동기화**를 선택하세요.
Google 토큰은 현재 실행 중에만 유지하므로 재방문·만료 시 재연결이 필요할 수 있습니다. 로그인 없이도 일반 읽기는 가능합니다.

메인 안내를 닫으면 이 브라우저에서는 다시 표시하지 않습니다. **동기화 → 저장 · 오프라인**에서 오프라인·백업·저장 보호를 계속 사용할 수 있습니다.
