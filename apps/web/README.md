...
- 서재 저장 보호·일주일 백업 알림, 마지막 백업 생성 시각·저장 여유 안내, 작업 중 종료/업데이트 보호.
...
2. 서재 안내의 **저장 보호 요청**을 누르고 파일을 가져옵니다. 승인은 브라우저 정책에 따릅니다.
3. **저장·오프라인 설정** 또는 동기화 버튼에서 **모든 형식 오프라인 준비**를 누릅니다.
   ‘모든 형식 준비됨’이 표시되면 처음 여는 TXT/EPUB/PDF/만화 화면과 import 엔진도 오프라인으로 준비됩니다.
   준비 전에는 기본 화면과 온라인에서 사용한 파일만 캐시에 있으므로 아직 열지 않은 화면은 인터넷이 필요할 수 있습니다.
4. **백업 및 복원 → 백업 만들기**로 ZIP을 받고 실제 다운로드 완료를 확인합니다. 백업 생성 시각은 이 브라우저에만
   기억하며 저장 취소/실패에는 갱신하지 않습니다. 생성 성공은 외부 드라이브에 영구 보관됐다는 보증이 아닙니다.
5. 지원 브라우저에서는 **앱 설치**, 그 밖에는 브라우저 메뉴의 **홈 화면에 추가**를 사용합니다.
...
연결된 Cloud Vault의 원본 opt-in을 활용하세요. 모든 형식 오프라인 준비는 책장 백업을 대신하지 않습니다.
