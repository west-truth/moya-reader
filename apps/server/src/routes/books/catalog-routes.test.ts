...
describe('book catalog routes', () => {
  ...
  it('atomically patches only requested metadata fields and emits the final snapshot', async () => {
    ...
    expect(update?.params?.slice(18)).toEqual(['book_1', 'user_test', null]);
  ...
  it('commits a soft-delete lifecycle update and its sync event on the same client transaction', async () => {
    ...
    const client = {
      query: vi.fn(async (sql: string) => {
        ...
        if (sql.includes('update library_books')) {
          return {
            rowCount: 1,
            rows: [{ id: 'book_1', deleted_at: '2026-07-13T00:00:00.000Z', metadata_revision: 3 }],
          };
        }
    ...
    expect(queries[1]).toContain('set deleted_at = now()');
    expect(queries[2]).toContain('insert into sync_events');
  ...
  it('rolls back and releases a missing-book deletion', async () => {
    ...
    expect(queries[0]).toBe('begin');
    expect(queries[1]).toContain('set deleted_at = now()');
    expect(queries[2]).toContain('select metadata_revision from library_books');
    expect(queries[3]).toBe('rollback');
    expect(client.release).toHaveBeenCalledTimes(1);
  ...
  it('restores a trashed book with revision fencing', async () => {
    ...
    const client = {
      query: vi.fn(async (sql: string) => {
        ...
        if (sql.includes('update library_books')) {
          return {
            rows: [{ id: 'book_1', updated_at: '2026-07-13T01:00:00.000Z', metadata_revision: 4 }],
          };
        }
        return { rows: [] };
      }),
    ...
    expect(queries[1]).toContain('deleted_at = null');
    expect(queries[2]).toContain('insert into sync_events');
