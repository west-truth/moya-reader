...
import { coverContentRevisionConflict, generatedCoverCanReplace, isWritableCoverProvenance } from './cover-routes.js';
...
describe('hosted generated cover policy', () => {
  ...
  it('accepts tokenless legacy cover writes only before a book id has been purged and reused', () => {
    expect(
      coverContentRevisionConflict({ active_content_revision_id: 'revision-1', has_prior_purge: false }, undefined),
    ).toBeUndefined();
    expect(
      coverContentRevisionConflict({ active_content_revision_id: 'revision-2', has_prior_purge: true }, undefined),
