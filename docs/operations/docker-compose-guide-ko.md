...
- 업데이트 전 PostgreSQL과 MinIO의 기존 백업을 보관하고, 사용 중인 Compose 구성의 `web`, `api`, `worker`를
  같은 버전으로 함께 갱신한다. 이전 Web 화면은 새로고침한다.
- 롤백 전에 열린 브라우저의 IndexedDB reader v39 / source v6와 이미 실행된 PostgreSQL 0037–0040을 유지한다.
  사이트 데이터를 지우거나 기존 작품을 다시 가져올 필요가 없다. 추가 migration 0041은 기존 만화의 회차
  식별자와 마지막 읽기 위치에 대한 호환 보정이며 원문을 다시 쓰지 않는다.
- TXT·EPUB·만화의 읽음 표시는 실제 기록된 회차만 사용한다. 예전 버전에 저장되지 않은 독서 이력은 추측하지
  않으므로, 과거에 읽었더라도 기록이 없는 회차는 안 읽음으로 보일 수 있다.
- 영구 삭제 뒤 소스의 잔여 연결은 소스 재진입/새로고침 때 정리된다. 휴지통에 있는 작품의 연결은 복원을 위해
  유지한다. 소스에서 서버 업데이트 안내가 나오면 Web과 API 버전부터 확인한다.
- 배포 후 기존 작품 열기, 휴지통 복원/영구 삭제 후 같은 회차 다시 받기, 2화에서 6화로 이동했을 때 3–5화가
  안 읽음으로 남는지를 확인한다. 자동 검증은 로컬 저장소·실제 PostgreSQL/API와 합성 소스를 사용하며,
  개인 Suwayomi 확장과 물리 모바일의 동작까지 대신 확인하지는 않는다.
