...
- 선택한 회차는 기본 최대 2개씩 받아 처음 화부터 하나씩 저장합니다. 어댑터는 작품별 동시 처리를 1개로 제한할 수
  있습니다. 완료된 회차는 나머지 다운로드 중에도 열 수 있으며 취소·실패 전에 완료한 회차는 유지합니다.
- 텍스트 소스와 Suwayomi 작품 목록은 큰 표지·카드·목록 표시를 선택할 수 있습니다. 내려받은 회차는 개별 또는
  선택한 여러 회차를 한 번에 삭제할 수 있고, 남은 회차와 작품 연결은 유지합니다.
- Web/PWA의 뒤로가기는 읽기 → 작품 상세 → 이전 소스 목록 순으로 복원합니다. 회차 검색·정렬·페이지와 목록
  스크롤도 현재 탭에서 유지하며, 첫 앱 화면에서는 브라우저의 이전 페이지로 정상 이동합니다.
...
브라우저를 닫은 뒤 다운로드 자동 재개, 미다운로드 다음 회차 자동 가져오기와 수동 읽음 변경은
