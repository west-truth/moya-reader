...
## 위 스크롤·이어읽기·삽화 재진입 회귀 수정 (2026-09-10)

- PR #38 검사는 scrollTop 변화가 없어도 본문 자체가 밀리는 문제를 놓쳤다. 실제 소스 행을 비교하니 두 엔진에서
  약 80px 이동이 재현됐다. 이어읽기 응답 대기 중 사용자 스크롤을 덮어쓰는 경로도 확인했다.
- 앞쪽 overscan을 복원하고 가변 높이 보정은 행/목록의 좌표로 처리한다. 터치·관성 중에는 보정 scroll write를
  하지 않고, 멈춘 뒤 같은 layout commit에서 실제 위치와 좌표를 정리해 읽던 줄을 유지한다.
- 사용자 조작은 늦은 이어읽기/anchor 복원을 취소한다. 삽화는 다시 로드·디코딩하는 동안 기존 행 높이를 유지한다.
  이전 삽화 컴포넌트만 사용한 비교 검사에서 585→220px 축소가 실패로 재현됐다.
- 새 `reader-scroll-geometry-smoke.mjs`와 `--webkit` 모두 통과: 120/12,000문단, 프레임별 본문 이동 0px,
  멈춘 뒤 좌표 정리 오차 1px 미만, 이어읽기 취소, 실제 삽화 재로딩 후 줄 이동 0px, 측정 중 처음으로 이동 통과.
- 기존 `reader-position-smoke.mjs`의 끝 도달/다음 제스처/높이 증가·축소/손가락 유지/반대 방향 이동,
  긴 회차 캐시 재로딩과 모드 왕복도 두 엔진에서 통과했다. 회귀 검사는 실제 소스 위치·행 높이를 기준으로 한다.
- 공개 `pnpm check:web-server` 전체 exit 0: deploy 314, 외부 소스 211, 복구 103, 가져오기 89/조건부 skip 3,
  Reader UX 85, 타입/lint/format/CSS/라이선스/경계, 서버 production, Hosted 264 검사, Web build 통과.
- 이전 검사 통과를 실제 iPad/Android 관성 체감 확인으로 해석하지 않는다. 실기기와 사용자 작품 검증은
  서버 배포 후 남아 있다. 아래 PR #38 기록은 당시의 결과이며, 그때의 overscan/보정 방식은 이 수정으로 대체됐다.
