...
작성일: 2026-09-23. 최초 검토 기준: `40ee131` (PR #61 병합 상태).
현재 상태: **포터블 배포물과 기본 실행 경로 구현·Windows 검사 완료, 대용량 파일 보관과 작업 생명주기는 후속 구현 전**. 최신 확인 기준은 제품 코드 `8fdef61`, 검증 기록 `f5613f3`이다. 아래 초기 현황과 뒤쪽 진행 기록을 구분해 읽는다.
사용자 확정 사항: **설치 프로그램이 아닌 `Moya.exe` 단일 배포물, 포터블 실행**.

현재 진행 요약:

- 단일 EXE, 포터블 프로필, 확장 실행기·프록시, 시스템 브라우저 연결, 최근 리더/UI 수정은 구현했다. Windows 후보 빌드의 실제 검사 범위는 아래 진행 기록에 남겼다.
- 사용자 확인: `전능의 뇌를 얻었다` 연결은 ByeDPI를 켜면 정상이다. 권 단위 대용량 다운로드 문제와는 분리한다.
- 다음 작업은 **파일 기반 대용량 저장·백업, 작업 중 종료/절전 복구, 웹·앱 공통 원형 진행률**이다. [상세 구현 계획](2026-09-23-desktop-storage-jobs-progress-plan.md)의 단계 A~E로 진행한다.
- 상세 계획은 구현 완료 보고가 아니다. 데스크톱의 기존 Blob/IndexedDB 백업 제한이 아직 해소된 것은 아니다.
...
아래 표는 `40ee131`에서 출발한 초기 현황이다. 이후 해결한 항목은 뒤쪽 진행 기록에 있으며, 아직 남은 저장·작업 문제의 최신 현황은 위 상세 구현 계획을 기준으로 한다. 과거 빌드 성공을 이후 변경의 검증으로 간주하지 않는다.
...
- [x] Desktop에 없는 native 파일 저장 기능을 플랫폼 capability에서 제공한다고 표시하던 오류를 수정했다. 실제 desktop 파일 선택/저장 adapter는 P2에 남아 있다.
...
- [x] [Windows 후보 #35815470091](https://github.com/west-truth/moya-reader/actions/runs/35815470091): 동봉 WebView2를 포함한 EXE 빌드는 통과했으나, 후속 smoke 명령이 루트에서 `tsx`를 찾아 실패했다. `tsx`가 있는 `apps/server` workspace에서 실행하도록 수정했다. 이 run은 실행 검증 성공으로 세지 않는다.
- [x] [Windows 후보 #35816290511](https://github.com/west-truth/moya-reader/actions/runs/35816290511): CAB 약 309MB를 Rust `include_bytes!`에 넣은 후보가 Windows rustc/LLVM 메모리 부족으로 실패했다. 압축된 Node/수집기 payload만 Rust에 포함하고, CAB는 컴파일 후 EXE 뒤에 고정 길이·해시 footer와 함께 붙이도록 분리했다. 실제 빌드·실행 성공 여부는 다음 후보에서 확인한다.
- [x] [Windows 후보 #35817393929](https://github.com/west-truth/moya-reader/actions/runs/35817393929): CAB 분리 후 Rust 컴파일은 메모리 부족 지점을 넘겼으나, Windows 전용 CAB 읽기 코드에서 `File::by_ref()`의 `Read`/`Write` 메서드가 중복되어 실패했다. 남은 버퍼 길이만 직접 읽는 방식으로 수정했다. 실행 성공 증거는 아직 없다.
- [x] [Windows 후보 #35818050155](https://github.com/west-truth/moya-reader/actions/runs/35818050155): 새 형식의 단일 EXE 빌드·해시 출력·아티팩트 업로드 성공. 강제 fixed WebView2를 사용한 첫 실행이 Windows 종료 코드 `0xC00000FD`(stack overflow)로 끝났다. 원인은 아직 확정하지 않았다. 재컴파일 없이 같은 EXE를 검사하도록 workflow에 `artifact_run` smoke 전용 입력을 추가하고, 일반 WebView2 시작과 강제 fixed 시작을 분리해 진단한다. **첫 실행 성공/릴리즈 가능으로 표시하지 않는다.**
- [x] [아티팩트 재검사 #35818945317](https://github.com/west-truth/moya-reader/actions/runs/35818945317): 강제 fixed 옵션이 없는 일반 시작에서도 `MoyaData/runtime` 준비 직후 메인 스레드 stack overflow가 재현됐다. `ensure_webview2()` 내부의 1MiB 고정 배열이 Windows GUI 메인 스레드의 작은 스택을 소모하는 것으로 추정해 256KiB 힙 버퍼로 바꿨다. 수정 후보의 결과로 원인을 최종 확인한다.
- [x] 포터블 runtime 버전 키에 Node/수집기 payload와 고정 WebView2 CAB 해시를 모두 포함한다. 후속 버전에서 CAB만 갱신하더라도 이전 fixed runtime 폴더를 잘못 재사용하지 않는다.
- [x] P0 검증 보강: Windows 후보 workflow에 EXE 첫 실행, 한글/공백 경로의 포터블 실행기 준비, 두 번째 실행, 앱 종료 후 폴더 이동·재실행 smoke를 추가했다. 첫 실행은 시스템 WebView2가 있어도 고정 버전 추출을 강제로 거쳐 fallback 경로를 검사한다. 뒤이어 **EXE에서 실제로 해제한** Node 실행기와 JS 소스로 기존 native smoke를 실행한다. 실제 통과 여부는 다음 Windows run 결과로 기록한다. 이 검사는 라이브 사이트·대용량 백업이나 실제 WebView2 미설치 PC를 검증하지 않는다.
...
사용자 확인: 필요한 배포물은 설치 프로그램 없는 `Moya.exe` 하나다. 모든 PC의 최초 오프라인 실행까지 보장하지 않는다.

- WebView2 Fixed CAB 동봉과 EXE overlay를 제거하고 시스템 Evergreen Runtime을 사용한다. 없으면 공식 설치 안내를 표시한다.
- 포터블 수집기를 onedir로 묶고 Python Playwright가 소스 실행기의 Node를 공유한다. 외부 배포물은 계속 단일 EXE다.
- Windows 후보는 수동 실행으로 바꾼다. 실제 UI 준비 신호, 폴더 이동 후 설정 보존, 공유 Node/Python 드라이버 연결만 추가 확인한다.
- 보관소는 host 재시작 없이 유휴 시점에 전환한다. 최초 암호 설정 전에 저장한 세션 설정을 암호화 보관소로 옮기며, 설정된 보관소가 잠긴 상태에서는 직접 연결로 되돌리지 않는다.
- 필수 Mangayomi host 초기화 실패와 목록 갱신 실패를 숨기지 않는다. 실패한 호환 소스의 이전 목록은 사용 가능 목록에서 제외한다.
- 이전 실행기 폴더 자동 삭제, 파일 기반 대용량 저장/백업(P2), 절전·다운로드 복구는 이번 수정에 섞지 않는다. 기존 runtime 폴더는 자동 정리하지 않으므로 업데이트 후 잔여 디스크 사용은 남는다.
...
- 로컬 회차와 확장 소스 회차의 페이지 변경에서 제목으로 강제 초점 이동·`scrollIntoView`를 제거했다. 페이지 버튼으로 연속 조작할 수 있으며 웹·데스크톱에 공통 적용된다. 짧은 마지막 페이지에서 돌아올 때도 버튼이 밀리지 않도록 공통 페이지 컴포넌트에서 버튼의 화면 좌표를 기준으로 해당 스크롤 컨테이너를 보정한다.
- 집중 검사: 브라우저 선택 3개 + 기존 회차 화면 19개 = 22개 통과. Windows 후보의 실제 브라우저 실행 결과는 아래에 추가한다.

브라우저 조작 확인: 격리 Chromium에서 로컬·소스 회차 목록 각각 390px/1280px, 다음 → 짧은 마지막 페이지 → 이전 페이지의 총 12회 이동을 검사했다. 페이지 버튼이 뷰포트 안에 유지되고 좌표 차이는 최대 1px 미만이었다. 테스트 컨테이너는 종료했으며 운영 서비스에는 변경하지 않았다.
...
- [새 Moya.exe](https://github.com/west-truth/moya-reader/actions/runs/35824714619/artifacts/10734942361): 98,263,040바이트(93.71MiB), SHA-256 `79a90dbed6e5f672276ea4c3c67d83a8d78ca03578c7c246928793e97097f8f9`.
- 동봉 Node + 제한된 환경변수로 Playwright·Patchright 각각 실제 설치 브라우저 실행 및 페이지 스크립트 처리 성공. 앱 화면 시작·설정 보존·소스 실행기·수집기 기존 검사도 통과했다. 실제 사이트 전체 호환성이나 사용자 PC의 터치패드 동작까지 검증했다는 뜻은 아니다.
- Linux에서 변경 관련 22개 테스트, 앱/서버/스크립트 타입 검사, 변경 TS lint, Rust 포터블 컴파일 통과. UI 보완 후 관련 19개 테스트 재확인 및 390px/1280px 페이지 왕복, 720px/1280px 창 경계 스크롤 확인.
- Windows 후보로 배포했으며 운영 웹 Docker 서비스는 변경하지 않았다. 회차 페이지 수정은 공통 웹 코드에도 포함되어 다음 웹 배포 때 반영된다.

### 페이지 전체 맞춤 비율 수정

self-host와 데스크톱은 `FixedDocumentScreen.tsx`와 `fixed-document.css`를 공유한다. 만화 이미지에 원본 `width`/`height` 속성이 설정된 상태에서 페이지 전체 맞춤이 `max-width`/`max-height`만 각각 제한해 비율이 왜곡됐다. 1000×2000 이미지가 1280×820 화면에서 1000×660으로 표시되는 현상을 실제 CSS로 재현했다. 높이 맞춤은 이미 `width: auto`를 지정해 이 문제가 드러나지 않았다.

페이지 전체 맞춤에 `width: auto; height: auto`를 지정해 자연 비율을 유지한다. 수정 후 같은 이미지가 330×660으로 표시된다. 격리 Chromium에서 웹/데스크톱 프레임, 모바일/PC 폭, 세로/가로 이미지의 8조건을 확인했다. 포맷 및 diff 검사 통과. 이 CSS 후속 수정은 코드에 커밋했으며 위 `2a8affb` EXE에는 아직 포함되지 않는다. 웹 운영 서비스도 변경하지 않았다.

### 소스 상세·빠른 보기·두쪽 보기 후속 수정

- 소스 상세 상단은 2열 grid에 제목/설명 배지/작업 버튼의 3개 항목을 넣어 톱니바퀴가 다음 줄로 밀렸다. 불필요한 설명 배지를 제거해 다운로드 중에도 버튼을 상단 한 줄에 유지한다.
- 텍스트 빠른 보기의 header 규칙이 뒤에 로드되는 공통 `.modal header`와 우선순위가 같아서 음수 margin에 덮였다. 해당 dialog의 header 선택자만 구체화해 제목·닫기 버튼 여백을 복구한다.
- 두쪽 보기의 첫 장: 중간에서 역방향으로 계산할 때 제목 높이를 포함하는 순간 첫 문단 `1화`를 `1` / `화`로 갈랐다. 1번 문단이 `1화`인 합성 본문에서 8번 문단 이어보기 → 이전 이동으로 사진과 같은 빈 왼쪽 페이지를 재현했다. 역방향 계산이 실제 첫 페이지 영역에 도달하면 첫 페이지를 정방향으로 다시 채운다. 한 문단에 긴 본문이 들어 있는 경우 중간 위치를 처음으로 되돌리지 않는다. 원문을 삭제하거나 생략하지 않으며, 이후 페이지는 새 끝 위치에서 이어진다. 페이지 측정 버전을 올려 예전 잘못된 경계를 재사용하지 않는다.

필요 범위 검증: `reader-page-window`, `PaginatedReaderViewport`, `SourceHubScreen` 3파일/32개 테스트, 앱 TypeScript, 변경 파일 포맷·diff 검사 통과. 격리 Chromium + 실제 앱 CSS/데스크톱 프레임에서 1280/1810px 상단 버튼이 헤더 안에 들어오는지 확인했다. 빠른 보기 제목·닫기 여백을 이미지로 확인했고, 4~9번째 문단에서 이어 읽다 앞으로 돌아오는 6조건 모두 첫 문단이 갈라지지 않았다. Windows WebView에서 동일한 실사용 작품을 조작한 결과와 구분한다.

#### ‘전능의 뇌를 얻었다’ 표지·본문 진단

2026-09-23, 토끼 소설 0.2.7 원본 확장을 임시 저장소에 설치해 확인했다. 운영 서재/설정은 변경하지 않았다.

- 실제 표지 CDN `aws-cdn9.site`: 직접 연결은 TLS 연결 중 reset, 기존 ByeDPI SOCKS 경유는 HTTP 200·원본 PNG 3,261,880바이트. 같은 파일의 경로 차이이며 표지 용량 제한 때문이 아니다.
- 실제 Moya Mangayomi host + 프록시: 작품 검색, WebP 목록 표지 49,304바이트, 314회차 목록 성공. Chromium이 있는 격리 컨테이너에서 1화 본문 10,631바이트도 성공했다. 다운로드 전체/모든 회차 및 사용자 Windows에서의 성공을 뜻하지 않는다.
- 브라우저가 없는 Linux 호스트의 본문 요청은 `source_browser_unavailable`로 실패했다. 데스크톱의 브라우저 경로 수정은 앞선 EXE에 이미 포함되어 있지만 사용자 PC의 브라우저/네트워크 상태는 별도다. WebView2 화면이 열린다고 사이트 처리용 Edge/Chrome이 정상 실행된다는 뜻은 아니다.
- self-host와 desktop의 **확장 회차** 상한은 공통: 이미지 2,048장·합계 256MiB·개별 20MiB, 텍스트 2MiB, 다운로드 10분. 대용량 로컬 파일 가져오기 한도와 구분한다. 데스크톱 서재는 여전히 WebView 저장소이고 입력 파일 3배+8MiB의 여유 quota를 확인하므로 서버 디스크 저장과 여유 공간 판단은 다르다. 이번 작은 회차가 이 한도에 걸린 증거는 없다.
