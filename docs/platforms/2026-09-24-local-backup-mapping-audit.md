...
- [Windows 앱 창 검사 35948632633](https://github.com/west-truth/moya-reader/actions/runs/35948632633), 코드 `9fe6097`에서 실제 WebView의 파일 입력에 로컬 v1 ZIP을 지정하고 검사 안내·복원 버튼을 거쳐 서버의 원본 바이트·독서 위치·북마크를 확인했다. Playwright가 파일 입력을 설정했으므로 OS 파일 선택 대화상자 자체는 검사하지 않았다.
- 남은 조건: 다양한 기존 사용자 ZIP, 프로세스 강제 종료 뒤 재시도, OS 파일 선택 대화상자, 256MiB 근처 메모리 측정. 원본 ZIP은 복원 과정에서 수정하지 않으며 실패 후 재시도는 객체 업로드 실패 주입으로만 확인했다. 변환기에서 거부한 저장소의 필드 계약을 조사해 작은 범위씩 추가한다.
