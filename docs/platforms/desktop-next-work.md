...
상태: **0.1.1-beta.1 최종 후보 검증 중 · 공개 바이너리 배포 게이트 미완료** · 2026-09-26.
제품 구조의 기준은 [desktop.md](desktop.md)입니다. 이 문서는 그 구조를 구현·검증할 다음 워커의 실행 순서를 정합니다.
진행 결과는 이 문서에 갱신하며, 별도 날짜별 계획을 계속 만들지 않습니다.

**현재 결과 확인:** [PR #62](https://github.com/west-truth/moya-reader/pull/62)의 최신 검증 항목과 [베타 후보 안내](../releases/0.1.1-beta.1.md)가 기준입니다. 아래 날짜별 검토 기록의 미검증 표시는 당시 상태입니다. 이미 끝난 작업을 다시 구현하지 않습니다.
...
### 사용자 글꼴 로딩 수정 (2026-09-26)

- Windows 내장 서재의 글꼴 추가는 등록 전 `FontFace`로 선택 파일을 검사한다. native CSP의 `font-src`에 `blob:`이 없어 정상 TTF도 이 단계에서 거절됐다. 저장된 사용자 글꼴의 본문 적용에도 같은 제한이 있었다.
- `font-src`에 `blob:`만 추가했다. 실제 PDF.js 배포 글꼴을 사용해 수정 전 NetworkError, 수정 후 검사·적용·해제·다시 적용 성공을 Chromium에서 확인했다. `user-font-csp.test.mjs`와 기존 표지 CSP 검사를 browser CI에 연결했다.
- 파일 형식/크기 오류, 글꼴 로딩 오류, 저장/목록 갱신 오류를 구분해 표시한다. 저장 실패로 기존 선택 글꼴을 바꾸지 않는 검사도 추가했다. 관련 단위 검사 3개와 CSP 브라우저 검사 2개, 웹 타입·변경 파일 lint 통과.
- native 보안 설정 변경이므로 기존 `e06717a` 테스트 EXE에는 반영되지 않는다. 수정본 Windows 설치 EXE를 새로 빌드해야 적용된다. 이 변경의 Windows 실사용 검증은 아직 하지 않았다.

### 리더 조작·선택 하이라이트 수정 (2026-09-26)

- 텍스트 리더의 몰입 모드 버튼을 자동 스크롤 설정 버튼으로 바꿨다. 처음 몰입 모드로 열리는 동작은 유지하고, 중앙을 눌러 도구를 열면 시간 경과로 숨기지 않는다. 다음 중앙 탭으로 다시 몰입 모드에 들어간다. 문서·만화의 중복 몰입 버튼도 제거하고 기존 중앙 전환과 자동 읽기 버튼을 유지했다.
- 도구 모음 자동 숨김이 없어져 의미가 없어진 `도구 모음 항상 표시` 설정은 화면에서 제거했다. 저장된 설정 필드와 구버전 호환은 유지한다.
- 선택 없는 하이라이트는 비활성화하고 저장 경로에서도 거부한다. 여러 문단의 선택은 문단마다 실제 선택한 인용문으로 나누어 저장한다. 역방향 선택·터치 selectionchange·페이지에 나뉜 문단을 처리하고, 인용문을 찾지 못했을 때 문단 전체를 칠하던 fallback을 제거했다.
- 색 버튼은 적용/변경이며 같은 색을 다시 눌러도 삭제되지 않는다. 선택 도구에 삭제 버튼을 두어 겹치는 하이라이트를 지운다. 저장 중 중복 입력을 막고 일부 저장 실패 시 실제 저장 목록을 갱신하며 선택을 유지해 재시도한다. 기존 주석 목록의 ID별 삭제는 유지한다.
- 기존 quote 기반 저장 형식을 사용한다. 과거 잘못 저장된 인용문을 추측해서 자동 변경하지 않으며 주석 목록에서 삭제할 수 있다. 동일 문단에 똑같은 인용문이 반복될 때의 정확한 위치 지정은 기존 저장 형식의 제약으로 남아 있다.
- 검증: 리더 회귀 109개, 변경 집중 검사 23개 통과(일부 중복). 실제 Chromium에서 정/역방향 여러 문단 선택·저장·삭제, 모바일 리더의 중앙 전환 유지·자동 스크롤 진입·선택 도구 가로 넘침 부재, 기존 스크롤/페이지 전환 검사를 통과했다. 웹 타입·변경 파일 lint·CSS·형식 검사도 통과했다. 관련 브라우저 검사를 PR CI에 추가했다.
- 기존 테스트 EXE에는 글꼴 수정과 이번 변경이 없다. 새 Windows EXE 빌드와 Windows 실사용 검증은 수행하지 않았으며 기존 성공 기록과 구분한다.
