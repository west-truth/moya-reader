...
현재 확인한 것은 관련 90개 검사와 저장 경로 묶음 41개, 타입·린트·서버 production build입니다. **현재 정리본의 실제 Windows 실행은 아직 확인하지 않았습니다.**
...
| ------------------------------ | ------ | --------------------------------------------------------------------- |
| A 정리본 Windows 실행          | 미실행 | 현재 코드 기준 `cb833c8`; 과거 Windows 성공을 이 칸에 재사용하지 않음 |
