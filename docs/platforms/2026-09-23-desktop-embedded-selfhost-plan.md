...
작성: 2026-09-23. 상태: **P0 완료. P1 후보 앱의 Windows 자동 연결·리더·공유·재시작 검증 통과, Cloudflare 기본 접속 추가. P1 배포/복구 검증은 진행 중**.
...
- [x] P0의 Windows 실행 구성과 서버·웹의 실제 사용 흐름 검증. Tauri 앱 창 연결은 P1에 남음.
- [ ] P1 접속 가능한 앱.
- [ ] P2 주요 기능 연결·이전·데이터 보호.
- [ ] P3 동기화·간편 접속.

브랜치 분리와 계획 재작성 후 사용자 승인으로 P0 구현을 시작했다. Tauri 자동 연결·시작 오류·트레이/종료·사설망 접속 허용·공통 QR 화면을 구현했고 Windows 후보 앱의 실제 사용 검사를 통과했다. Cloudflare Quick Tunnel을 기본 방식으로 추가했고 Linux에서 실제 HTTPS 로그인·해제를 통과했다. 실제 별도 기기, 설치 필수 구성 자동 준비, 전원 종료 복구와 퍼센트 UI 이식은 아직 완료하지 않았다. 기존 Docker 운영 서비스와 다른 작업자의 변경은 이 작업에서 수정하지 않았다.
