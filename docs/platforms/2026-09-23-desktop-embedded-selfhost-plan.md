...
**현재 실행 위치:** [실행 지침과 작업별 인계](2026-09-24-desktop-next-work-handoff.md)의 진행 기록을 따른다. A1~A3 수집기 연결과 B의 실제 앱 읽기·검색·백업 흐름, C1 기본 로컬 ZIP 앱 창 복원은 Windows에서 검증했다. D1은 두 합성 서버에서 빈 서재 초기 백업 복제와 신규 독서 위치 전달을 검증했다. B의 남은 장애 경계, C1의 다양한 자료, 최초 복제 이후의 작품·원본 변경과 E 일반 배포 검증은 남았다. 새 작업자는 이 전체 계획과 실행 지침을 함께 읽는다.
