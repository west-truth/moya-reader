...
작성: 2026-09-23. 상태: **P0 서버·웹 사용 흐름 검증 완료. P1 앱 연결·공통 원격 접속 화면 구현 및 Windows 검증 진행 중**.
...
- 기본 접속 범위는 이 PC다. `다른 기기 접속 허용`을 켜면 지정한 인터페이스로 제공하고, 접속 주소·인증 방법·현재 상태를 표시한다. 공유 해제 시 외부 접근이 중단되고 이 PC 읽기는 유지된다.
...
브랜치 분리와 계획 재작성 후 사용자 승인으로 P0 구현을 시작했다. Tauri 자동 연결·시작 오류·트레이/종료·사설망 접속 허용·공통 QR 화면을 구현해 검증 중이다. 실제 별도 기기, 설치 필수 구성 자동 준비, 전원 종료 복구와 퍼센트 UI 이식은 아직 완료하지 않았다. 기존 Docker 운영 서비스와 다른 작업자의 변경은 이 작업에서 수정하지 않았다.
