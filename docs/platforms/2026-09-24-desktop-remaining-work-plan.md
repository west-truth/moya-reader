...
| W02a    | TXT/Markdown 한 차례 원본 교체 구현, 나머지 포맷·오프라인 연속 변경은 남음 | 실제 독립 2 DB/HTTP에서 import worker 원본 교체, 대상 독서 위치·메모·제목 보존, 재전송·원본 누락 복구·동시 교체 충돌·revision 고정 다운로드·수신 기준 변경 거부 통과. 과거 원본 객체 GC와 전체 백업 포함 검사 통과. commit 직전 실패 주입은 남음 |
| W02b~W08 | 후속 구현 필요 | W02a 결과를 전체 원본/주석 동기화로 표시하지 않음 |
