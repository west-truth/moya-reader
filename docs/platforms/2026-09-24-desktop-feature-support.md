...
| 독립 서버 간 동기화 | D1 독서 위치 첫 범위 구현 | 같은 작품·원본 hash·revision·회차 ID가 이미 양쪽에 있는 경우 신규 독서 위치 이벤트만 서버 간 전송. 암호화한 계정 세션·방향별 cursor·백그라운드 실행, 재전송/재시작/충돌/미지원 이벤트/세션 만료를 두 DB 통합 검사로 확인. 초기 작품/원본 복제와 일반 사용자 UI는 없음. [계약·제한 기록](2026-09-24-server-sync-contract-audit.md) 참고 |
