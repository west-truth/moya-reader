...
| 독립 서버 간 동기화 | D1 빈 서재 복제·독서 위치 첫 범위 구현 | 기존 서버 백업으로 빈 대상 서재의 원본·자산·기존 상태를 한 번 복제하고, 이후 같은 작품·원본 hash·revision·회차 ID의 신규 독서 위치 이벤트만 서버 간 전송. 암호화한 계정 세션·방향별 cursor·백그라운드 실행, 실패 후 복제 재시도와 재전송/재시작/충돌을 두 DB 통합 검사로 확인. 이후 작품·원본 변경과 일반 사용자 UI는 없음. [계약·제한 기록](2026-09-24-server-sync-contract-audit.md) 참고 |
