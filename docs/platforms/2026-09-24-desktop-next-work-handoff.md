...
- **D1 공통 연결 화면:** 앱 정보 설정에서 기존 서버 계정으로 연결→빈 서재 초기 복제→상태 조회·재시도·즉시 독서 위치 동기화·해제를 공통 원격 API 클라이언트로 호출한다. 백업 응답에 전체 바이트가 있으면 실제 받은 바이트로 숫자 퍼센트를 표시한다. 현재 스트리밍 ZIP은 전체 바이트를 제공하지 않아 실제 첫 복제에서는 받은 바이트 수·단계를 표시한다. 컴포넌트 검사는 통과했다. 실제 Windows WebView/물리 타 기기에서 이 연결 화면의 입력·장시간 복제를 끝까지 조작한 검증은 남았다. 계속 추가되는 작품·원본과 다른 항목의 증분 동기화도 아직 없다.
