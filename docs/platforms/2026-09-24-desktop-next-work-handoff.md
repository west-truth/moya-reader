...
**현재 상태: D0 계약 조사, D1 독서 위치 첫 범위와 빈 서재 초기 백업 복제 통과. 공통 연결 화면 구현·컴포넌트 검사와 Windows 작은 TXT UI 복제 통과. 대용량·물리 타 기기와 이후 작품·원본 변경·다른 항목의 증분 복제는 미완료.**
...
> `/home/koho5155/Docker_Services/.worktrees/moya-desktop-selfhost`의 `feat/desktop-embedded-selfhost`에서 작업한다. 전체 구현 계획, 이 인계 문서, C0/D0/D1 조사 문서를 읽는다. A1~A3과 `4dcbaf6`까지의 Windows 실제 앱 검사를 재구현하지 않는다. C1 기본 로컬 ZIP 변환은 서버 staging에 있으며 미지원 행을 조용히 버리지 않는다. C1의 다양한 사용자 자료 검증을 넓히고, D1 초기 복제 이후 작품/원본 변경과 대용량·타 기기 복제, B의 남은 종료·디스크 부족·AI/TTS·실제 공유 단절, E 배포 검증을 진행한다. 기존 self-host와 공통 UI를 재사용하며 데스크톱 IDB 정본이나 새 수집 엔진을 만들지 않는다. 작업 단위별 검증 결과와 다음 작업 ID를 기록한다.
