...
작성: 2026-09-24. 초기 코드 확인 기준: `d537b26`. 현재 수집기 검증 기준: `775a9ff`, 실제 앱 검증 기준: `48b62c5`. **최신 남은 작업·실행 순서는 [전체 후속 작업 계획 W01~W12](2026-09-24-desktop-remaining-work-plan.md)을 따른다.** 사용자 요청으로 물리 타 기기 검증은 이번 실행 범위에서 제외했다. 아래 A~E는 기존 구현·검증 기록과 코드 참고 자료이며, 완료 항목을 재구현하는 지시가 아니다.
...
- **D2a 새 작품 전달:** [작품 전송 계획·구현 기록](2026-09-24-peer-book-transfer-plan.md)에 따라 기존 백업 exporter/parser/restore를 한 작품의 콘텐츠만 전송하도록 연결했다. 작은 TXT의 양방향 전송·원본 보존·중복·실패 후 재시도·설정 보존을 실제 두 DB에서 검사했다. 관련 45개 검사 통과. 최종 [`48b62c5` Windows 실행 35963132505](https://github.com/west-truth/moya-reader/actions/runs/35963132505)에서 실제 가져오기→새 작품 전달·원본 바이트 확인과 전체 앱 회귀가 통과했다. 변경 번호/commit 순서 누락과 초기 복제 대기 중 설정 저장도 재현·수정했다. 다음 기능 단위는 D2b의 원본 교체 계약과 주석/메타데이터다.
- **직전 변경 재검토:** `a59cc9f`/`19a0d65`에서 독서 위치 삭제/과거 저장 경쟁, session 만료 후 cursor 보존 재로그인, 첫 복제 전 중단 복구, 바이트·검증 단계 표시를 수정했다. 관련 49개 검사와 서버·웹 타입 검사·변경 파일 린트를 통과했다. [재현·수정·검증 범위](2026-09-24-desktop-peer-review.md)를 먼저 확인하고 이 변경을 보존한다. 후속 Windows `19a0d65`의 [실행 35960786910](https://github.com/west-truth/moya-reader/actions/runs/35960786910)에서 실제 복제 재시도·재로그인·cursor 보존과 전체 앱 회귀가 통과했다.
...
- **D1 독서 위치 첫 범위:** 동일 book/source hash/active revision/chapter ID를 가진 두 독립 서버에서 새 독서 위치만 교환하는 API 실행기를 추가했다. 서버 DB의 방향별 cursor, 암호화한 상대 계정 session, 15초 백그라운드 실행을 사용한다. 두 실제 PostgreSQL DB/HTTP 서버에서 A→B, 재시작 후 B→A, 같은 시각 충돌·미지원 북마크·세션 만료를 검사했다. 이 첫 범위 뒤 빈 서재 최초 복제와 공통 연결 화면을 추가했다. 세부 계약은 [D0/D1 기록](2026-09-24-server-sync-contract-audit.md)을 따른다.
...
- **D1 빈 서재 최초 복제:** 기존 서버 백업 스트림을 계정 session으로 받아 대상 서버의 기존 staging/복원에 연결한다. 빈 대상 서재에 원본·자산·읽던 위치 등을 처음 복사하고 이후의 새 독서 위치 사건은 기존 D1 실행기가 교환한다. 복원과 수신 cursor는 같은 DB commit으로 처리한다. 두 실제 DB/HTTP 서버에서 원본 누락 실패→대상 보존→재시도→원본 바이트와 신규 위치 전달을 검사했다. 공통 UI도 추가했으며, 계속 추가되는 작품·원본과 다른 항목의 증분 동기화는 아직 없다.
- **D1 공통 연결 화면:** 앱 정보 설정에서 기존 서버 계정으로 연결→빈 서재 초기 복제→상태 조회·재시도·즉시 독서 위치 동기화·해제를 공통 원격 API 클라이언트로 호출한다. UI 연결 요청은 빈 서재 조건을 서버에 전달하고, 책이 있는 대상에서는 기존 연결을 바꾸지 않고 409로 거부한다. 백업 응답에 전체 바이트가 있으면 실제 받은 바이트로 숫자 퍼센트를 표시한다. 현재 스트리밍 ZIP은 전체 바이트를 제공하지 않아 실제 첫 복제에서는 받은 바이트 수·단계를 표시한다. 컴포넌트 검사와 두 DB 통합 검사는 통과했다. Windows WebView에서도 주소·계정 입력→작은 TXT 최초 복제→원본 바이트 확인→연결 해제를 통과했다. 장시간·대용량 복제와 물리 타 기기 검증은 남았다. 계속 추가되는 작품·원본과 다른 항목의 증분 동기화도 아직 없다.
- **최초 복제 백엔드 Windows 회귀:** [실행 35954550175](https://github.com/west-truth/moya-reader/actions/runs/35954550175), 코드 `e2789b5`에서 내장 서버·복구·앱 창·공유·패키징 독서가 통과했다. 이 실행은 새 공통 연결 화면을 아직 포함하지 않아 UI 증거로 사용하지 않는다.
- **공통 연결 화면 Windows 회귀:** [실행 35956308428](https://github.com/west-truth/moya-reader/actions/runs/35956308428), 코드 `eefcfc9`에서 실제 WebView의 연결 화면과 빈 서재 입력 폼 표시, 앱 종료·재시작·공유·백업 복원·EPUB/PDF까지 통과했다. 이 검사는 폼 입력과 실제 서버 간 장시간 복제 자체는 수행하지 않는다. 앞선 [실행 35955608940](https://github.com/west-truth/moya-reader/actions/runs/35955608940)은 화면과 읽기·복원 확인 뒤 첫 앱 종료가 60초 제한을 넘겨 실패했다. 재실행에서 재현되지 않아 원인은 미확정이며, 이후 실패 시 종료 단계 로그를 남기도록 검사 스크립트를 보완했다. 두 실행 모두 설치본 생성 옵션은 켜지 않았다.
- **Windows UI에서 첫 복제:** [실행 35957135370](https://github.com/west-truth/moya-reader/actions/runs/35957135370), 코드 `4dcbaf6`에서 별도 내장 서버에 작은 TXT를 가져온 뒤 실제 WebView의 주소·계정 폼으로 연결·복제했다. 대상 서버의 원본 바이트와 `ready` 상태, UI 해제를 확인했고 기존 앱 읽기·백업·재시작 및 패키징 서버 EPUB/PDF도 통과했다. 같은 Windows runner의 두 독립 프로필로 검증했으며 물리 타 기기·대용량 장시간 복제·새 설치본 검증은 아니다.
...
| D0 → D1… | 독립 서버 간 동기화                     | 지원표와 동일 작품의 신규 독서 위치 양방향 실행기, 빈 서재 초기 snapshot·원본 복제, 공통 연결 화면과 Windows 작은 TXT UI 복제 통과. D2a 새 작품 전달의 로컬 검사와 Windows 실제 앱 검사 통과. 기존 작품 원본 교체·주석·삭제·충돌 해결과 대용량·타 기기 검증 남음 |
...
**현재 상태: D0 계약 조사, D1 독서 위치 첫 범위와 빈 서재 초기 백업 복제 통과. 공통 연결 화면 구현·컴포넌트 검사와 Windows 작은 TXT UI 복제 통과. D2a 새 작품 콘텐츠의 양방향 전달과 로컬 DB 검사, Windows 실제 가져오기·전송 검사는 완료했다. D2b 기존 작품 원본 교체·다른 항목의 증분 복제와 대용량·물리 타 기기 검증은 미완료.**
...
4. 빈 서재의 초기 작품/원본 복제는 기존 서버 백업 경로로 구현했다. D2a에서 연결 후 새 작품의 원본 전달을 추가했다. 다음 단위에서 기존 작품의 원본 교체, 주석/메타데이터, 삭제/충돌을 작품 전송 계획의 순서로 확장한다. 수정 충돌에서 원본을 임의로 덮어쓰거나 시각 하나로 모든 데이터를 승자 결정하지 않는다. 기존 규칙이 없는 항목은 데이터 보존과 명시적 충돌을 우선하는 계약을 먼저 적는다.
...
## 이전 인계 지시문 — 최신 지시는 W01~W12 문서 사용

> `/home/koho5155/Docker_Services/.worktrees/moya-desktop-selfhost`의 `feat/desktop-embedded-selfhost`에서 작업한다. 전체 구현 계획, 이 인계 문서, C0/D0/D1 조사 문서를 읽는다. A1~A3과 `48b62c5`까지의 Windows 실제 앱 검사를 재구현하지 않는다. D2a 작품 전송 계획과 실패·수정·최종 통과 기록을 읽는다. C1 기본 로컬 ZIP 변환은 서버 staging에 있으며 미지원 행을 조용히 버리지 않는다. C1의 다양한 사용자 자료 검증을 넓히고, D2a 새 작품 전송을 보존하며 D2b 원본 교체와 대용량·타 기기 복제, B의 남은 종료·디스크 부족·AI/TTS·실제 공유 단절, E 배포 검증을 진행한다. 기존 self-host와 공통 UI를 재사용하며 데스크톱 IDB 정본이나 새 수집 엔진을 만들지 않는다. 작업 단위별 검증 결과와 다음 작업 ID를 기록한다.
