...
- **직전 변경 재검토:** `72e0e6e` 이후 작업 트리에서 독서 위치 삭제/과거 저장 경쟁, session 만료 후 cursor 보존 재로그인, 첫 복제 전 중단 복구, 바이트·검증 단계 표시를 수정했다. 관련 49개 검사와 서버·웹 타입 검사·변경 파일 린트를 통과했다. [재현·수정·검증 범위](2026-09-24-desktop-peer-review.md)를 먼저 확인하고 이 변경을 보존한다. 새 수정본의 Windows 실행 증거는 아직 없다.
...
| D0 → D1… | 독립 서버 간 동기화                     | 지원표와 동일 작품의 신규 독서 위치 양방향 실행기, 빈 서재 초기 snapshot·원본 복제, 공통 연결 화면과 Windows 작은 TXT UI 복제 통과. 대용량·타 기기 검증과 이후 작품·원본 변경, 주석·삭제·충돌 해결 남음 |
