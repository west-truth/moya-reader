...
작성: 2026-09-24. 초기 코드 확인 기준: `d537b26`. 현재 수집기 검증 기준: `775a9ff`, 실제 앱 검증 기준: `9fe6097`. **다음 작업은 B의 남은 장애 경계와 초기 복제 이후의 변경 전송·실제 UI 검증이다.** 이 문서는 기존 구현 계획을 실행 단위로 구체화한다. 제품 구현을 완료했다는 기록이 아니다.
...
| D0 → D1… | 독립 서버 간 동기화 | 지원표와 동일 작품의 신규 독서 위치 양방향 실행기, 빈 서재 초기 snapshot·원본 복제, 공통 연결 화면 구현. 실제 UI·타 기기 검증과 이후 작품·원본 변경, 주석·삭제·충돌 해결 남음 |
...
**현재 상태: D0 계약 조사, D1 독서 위치 첫 범위와 빈 서재 초기 백업 복제 통과. 공통 연결 화면 구현·컴포넌트 검사 통과. 실제 UI 조작과 이후 작품·원본 변경·다른 항목의 증분 복제는 미완료.**
...
> `/home/koho5155/Docker_Services/.worktrees/moya-desktop-selfhost`의 `feat/desktop-embedded-selfhost`에서 작업한다. 전체 구현 계획, 이 인계 문서, C0/D0/D1 조사 문서를 읽는다. A1~A3과 `9fe6097`까지의 Windows 실제 앱 검사를 재구현하지 않는다. C1 기본 로컬 ZIP 변환은 서버 staging에 있으며 미지원 행을 조용히 버리지 않는다. C1의 다양한 사용자 자료 검증을 넓히고, D1 연결 화면의 실제 앱 조작, 초기 복제 이후 작품/원본 변경, B의 남은 종료·디스크 부족·AI/TTS·실제 공유 단절, E 배포 검증을 진행한다. 기존 self-host와 공통 UI를 재사용하며 데스크톱 IDB 정본이나 새 수집 엔진을 만들지 않는다. 작업 단위별 검증 결과와 다음 작업 ID를 기록한다.
