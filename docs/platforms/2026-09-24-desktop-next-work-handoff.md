...
작성: 2026-09-24. 초기 코드 확인 기준: `d537b26`. 현재 수집기 검증 기준: `775a9ff`, 실제 앱 검증 기준: `4dcbaf6`. **다음 작업은 B의 남은 장애 경계와 초기 복제 이후의 변경 전송·대용량 복제 검증이다.** 이 문서는 기존 구현 계획을 실행 단위로 구체화한다. 제품 구현을 완료했다는 기록이 아니다.
...
- **D2a 새 작품 전달:** [작품 전송 계획·구현 기록](2026-09-24-peer-book-transfer-plan.md)에 따라 기존 백업 exporter/parser/restore를 한 작품의 콘텐츠만 전송하도록 연결했다. 작은 TXT의 양방향 전송·원본 보존·중복·실패 후 재시도·설정 보존을 실제 두 DB에서 검사했다. 관련 45개 검사 통과. Windows 실제 가져오기→새 작품 전달 검사를 추가했으며 결과는 후속 실행 기록을 확인한다. 다음 기능 단위는 D2b의 원본 교체 계약과 주석/메타데이터다.
...
| D0 → D1… | 독립 서버 간 동기화                     | 지원표와 동일 작품의 신규 독서 위치 양방향 실행기, 빈 서재 초기 snapshot·원본 복제, 공통 연결 화면과 Windows 작은 TXT UI 복제 통과. D2a 새 작품 전달의 로컬 검사 통과. 기존 작품 원본 교체·주석·삭제·충돌 해결과 대용량·타 기기 검증 남음 |
...
**현재 상태: D0 계약 조사, D1 독서 위치 첫 범위와 빈 서재 초기 백업 복제 통과. 공통 연결 화면 구현·컴포넌트 검사와 Windows 작은 TXT UI 복제 통과. D2a 새 작품 콘텐츠의 양방향 전달과 로컬 DB 검사는 완료했다. D2b 기존 작품 원본 교체·다른 항목의 증분 복제와 대용량·물리 타 기기 검증은 미완료.**
...
> `/home/koho5155/Docker_Services/.worktrees/moya-desktop-selfhost`의 `feat/desktop-embedded-selfhost`에서 작업한다. 전체 구현 계획, 이 인계 문서, C0/D0/D1 조사 문서를 읽는다. A1~A3과 `4dcbaf6`까지의 Windows 실제 앱 검사를 재구현하지 않는다. C1 기본 로컬 ZIP 변환은 서버 staging에 있으며 미지원 행을 조용히 버리지 않는다. C1의 다양한 사용자 자료 검증을 넓히고, D2a 새 작품 전송을 보존하며 D2b 원본 교체와 대용량·타 기기 복제, B의 남은 종료·디스크 부족·AI/TTS·실제 공유 단절, E 배포 검증을 진행한다. 기존 self-host와 공통 UI를 재사용하며 데스크톱 IDB 정본이나 새 수집 엔진을 만들지 않는다. 작업 단위별 검증 결과와 다음 작업 ID를 기록한다.
