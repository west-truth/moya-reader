...
작성: 2026-09-24. 코드 확인 기준: `d537b26`. **다음 시작 작업은 A1이다.** 이 문서는 기존 구현 계획을 실행 단위로 구체화한다. 제품 구현을 완료했다는 기록이 아니다.
...
Windows 전체 성공: [35911331300](https://github.com/west-truth/moya-reader/actions/runs/35911331300), 코드 `3e747f1`. 이후 `d537b26`의 과거 업로드 세션 파일명 누락 방어는 로컬 타입 검사·관련 31개 검사로 확인했다. CI가 최신 HEAD 전체를 실행했다고 쓰지 않는다.
...
> `/home/koho5155/Docker_Services/.worktrees/moya-desktop-selfhost`의 `feat/desktop-embedded-selfhost`에서 작업한다. 전체 구현 계획과 `docs/platforms/2026-09-24-desktop-next-work-handoff.md`를 읽고 A1부터 진행한다. 기존 self-host·수집기·gateway를 재사용하며 데스크톱 IDB 정본이나 새 수집 엔진을 만들지 않는다. A1의 서버 연결/인증을 검사한 뒤 A2의 실제 수집기 동봉·실행으로 이어간다. 완료된 설치/숫자 진행률/대용량/백업 기능을 재구현하지 않는다. C0/D0의 미확정 데이터 계약은 지원표를 작성한 뒤 구현한다. 작업 단위별 검증 결과와 다음 작업 ID를 기록한다.
