...
- **B 실제 앱:** 같은 `62514e5` 실행의 앱 창 검사에서 native TXT 검색·북마크, EPUB/PDF 읽기, 실제 UI 백업 ZIP 저장→새 프로필 3권·원본·북마크 복원, 공유·재시작이 통과했다. `1d8e368`은 숨겨진 검색 입력을 선택한 검사 오류, `f9022cb`은 WebView에서 다운로드 이벤트를 기다린 검사/저장 경계 오류였다. 스트림을 실제 파일에 쓰고 완료를 확인하도록 수정했다.
- **B 공유·포트 단절:** LAN/Tailscale 주소가 사라졌을 때 listener를 내리고, Cloudflare 프로세스가 살아 있어도 공개 경로가 3회 연속 실패하면 공유 상태를 내린다. 합성 네트워크 단절 검사에서 로컬 API 유지까지 통과했다. 저장된 API 포트가 점유되면 명시적인 오류와 원본 프로필 보존을 Linux 합성 검사로 확인했다. 해당 변경의 Windows 회귀 검사는 대기 중이다. 실제 네트워크를 물리적으로 끊은 검증도 남았다.
