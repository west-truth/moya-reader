...
`append_image_series`는 같은 사용자·작품의 활성 페이지 중 SHA-256, 크기, 형식이 같은 객체를 재사용한다.
HEAD로 실제 객체의 존재·크기·형식을 확인하며, 누락/불일치는 검증된 원본에서 다시 저장한다. 연결·인증 오류는
실패로 처리한다. HEAD는 본문 hash를 다시 계산하지 않으므로 기존 객체의 불변성과 DB의 hash를 신뢰한다.

새 asset ID가 기존 객체를 참조할 수 있으며 기존 회차 ID와 원본 CBZ 형식은 유지된다. 재사용한 객체는 새
업로드의 실패 정리 목록에 넣지 않고, 활성 참조 교체 transaction에서 삭제 예정 목록에서도 제외한다.
최종 영구 삭제 시에는 기존 참조 확인과 object-delete outbox를 통해 함께 정리된다. 전역 cache나 교차 작품
공유는 추가하지 않는다. 표지와 일반 TXT/EPUB 교체 저장은 이 재사용 대상이 아니다.

페이지 PUT만 줄어들며 전체 원본 읽기·병합·파싱·저장은 아직 남는다. worker의 `import_job_profile`에 단계별
시간과 새로 저장/재사용한 페이지 수·byte 수를 기록한다. `pnpm test:import-pages`는 실제 PostgreSQL과
loopback S3 HTTP fixture로 실패/취소/회차 기록/표지/삭제·복원 호환을 검사하며 실 MinIO 성능을 대신 측정하지 않는다.
