...
설치 없이 시작하려면 [브라우저 Web](https://west-truth.github.io/moya-reader/)을 사용하세요. 파일과 기록은
개인 브라우저에 자동 저장됩니다. 파일을 가져오면 바로 읽을 수 있습니다. 인터넷이 없는 곳에서도 읽고 싶을 때만
‘인터넷 없이 읽기 켜기’를 선택하세요. 백업·설치·저장 보호도 설정에서 선택할 수 있으며 필수 절차가 아닙니다.
사용한 형식은 자동 캐시하고 전체 오프라인 설정은 업데이트에도 유지합니다. 작업 중 업데이트 보호도 제공합니다.

Web은 시스템 음성을 제공하며 AI·외부 TTS·메타데이터 수집·서버 소스는 desktop/self-host가 필요합니다.
Dropbox/Google 연결은 배포자의 OAuth 앱 설정 후 사용할 수 있습니다. [Web 사용·배포 안내](apps/web/README.md)를 참고하세요.
...
Self-host에서 일반 EPUB·ZIP·CBZ는 파일당 최대 4GiB를 지원합니다. 다른 형식과 회차 추가는 기본 500MiB입니다.
만화 여러 화 선택은 한 화씩 저장하므로 회차 업로드 한도를 전체 선택 용량에 적용하지 않습니다.
