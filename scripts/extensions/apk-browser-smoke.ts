...
const output = resolve(root, '.tmp', `apk-browser-${Date.now()}`);
...
await build({
 ...
 let keySaved=false,enabled=false,endpoint='';
 const manager={preferences:async()=>({revision:snapshot.revision,privateOrigins:[],networkPolicy:'direct',groups:[{id:'123',title:'첫 번째 소스'},{id:'456',title:'두 번째 소스',unavailable:true}],fields:[{group:'123',key:'key',title:'접속 키',kind:'text',secret:true,configured:keySaved,disabled:!enabled},{group:'123',key:'enabled',title:'외부 서버 사용',kind:'boolean',secret:false,value:enabled},{group:'123',key:'endpoint',title:'서버 주소',kind:'text',secret:false,value:endpoint,disabled:!enabled}]}),
 savePreferences:async(pkg,revision,values)=>{if(revision!==snapshot.revision)throw Error('conflict');if(values.key!==undefined)keySaved=!!values.key;if(values.enabled!==undefined)enabled=values.enabled;if(values.endpoint!==undefined)endpoint=values.endpoint;snapshot.revision++;},
 list:async()=>structuredClone(snapshot),refreshRepository:async url=>{if(!snapshot.repositories.some(r=>r.url===url))snapshot.repositories.push({url,updatedAt:Date.now(),entries});},
 removeRepository:async url=>{snapshot.repositories=snapshot.repositories.filter(r=>r.url!==url)},
 inspectRepository:async(url,pkg,code)=>({id:pkg,revision:snapshot.revision,digest:'a'.repeat(64),pkg,code,version:'1.4.1',signers:['b'.repeat(64)]}),
 ...
 createRoot(document.getElementById('root')).render(<main style={{maxWidth:720,margin:'20px auto',padding:16}}><ApkExtensionsPanel manager={manager}/></main>);
`,
    resolveDir: root,
    loader: 'tsx',
...
try {
  ...
  const panel = page.getByRole('region', { name: 'APK 확장 관리' });
  for (const url of ['https://first.example/index.min.json', 'https://second.example/index.min.json']) {
    await panel.getByLabel('APK 저장소 주소').fill(url);
  ...
  await panel.getByLabel('APK 확장 검색').fill('이미지 소설');
  await panel.getByRole('button', { name: '설치 검토', exact: true }).click();
  const review = panel.getByRole('article', { name: 'APK 설치 검토' });
  ...
  await panel.getByLabel('외부 서버 사용', { exact: true }).check();
  assert(await panel.getByLabel('서버 주소', { exact: true }).isDisabled());
  await panel.getByRole('button', { name: '설정 저장', exact: true }).click();
  await page.waitForFunction(
    () => !document.querySelector<HTMLInputElement>('.compatibility-preferences input[type=password]')?.disabled,
  );
  await panel.getByLabel('접속 키', { exact: true }).fill('fixture-secret');
  await panel.getByLabel('서버 주소', { exact: true }).fill('http://192.168.1.20:8080');
  assert.equal(await panel.locator('.compatibility-preferences textarea').count(), 0);
  for (const width of [390, 768, 1024]) {
    await page.setViewportSize({ width, height: 900 });
    assert(await page.evaluate(() => document.documentElement.scrollWidth <= innerWidth), `options overflow ${width}`);
  ...
  assert.match((await panel.getByLabel('접속 키', { exact: true }).getAttribute('placeholder')) ?? '', /저장됨/);
  await panel.getByLabel('소스 선택', { exact: true }).selectOption('456');
  await panel.getByText('이 소스의 설정에는 아직 지원하지 않는 기능이 있습니다.').waitFor();
  await panel.getByLabel('소스 선택', { exact: true }).selectOption('123');
  assert.equal(await panel.getByLabel('서버 주소', { exact: true }).inputValue(), 'http://192.168.1.20:8080');
  ...
  console.log('PASS APK settings browser: ' + output);
} finally {
  await browser.close();
