...
- 모바일에서 화면 속 입력칸을 터치하면 모야의 입력창에 포커스가 이동한다. 작성 후 `입력`을 누르면 선택한 칸의
  내용을 교체한다. `보기/숨김`, `칸 비우기`, `한 글자 삭제`, `다음 칸`, `확인`을 사용할 수 있다.
- 로그인 팝업이 열리면 새 창으로 전환한다. 상단 창 선택으로 원래 사이트로 돌아가거나 현재 팝업을 닫을 수 있다.
  새 창이 열린 뒤 이전 창을 대상으로 대기하던 입력은 거절한다. 화면을 숨기거나 앱을 새로고침한 뒤에는
  설정의 `로그인 화면`으로 진행 중인 창을 다시 열 수 있다.
- `로그인 완료` 시 프로필을 디스크에 반영하고 `/data/auth/session-cookies.json`에 세션 쿠키를 원자적으로
  보관한다. 다음 브라우저 실행 때 복원하며, 인증 검색과 정상 종료 때 변경된 세션 쿠키도 반영한다.
  영구 쿠키와 사이트 저장 정보는 기존 `/data/auth/browser-profile`을 계속 사용한다.
- 추가 주소·토큰·환경변수나 볼륨 설정은 필요 없다. 위의 기존 Compose 파일 세 개로 Web/API/수집기를 함께
  다시 빌드한다. `metadata-collector-data` 볼륨을 보존해야 한다. 세션 파일과 프로필은 인증 비밀이므로 공개하거나
  저장소에 넣지 않는다. 일반 Moya 백업/기기 동기화에는 포함하지 않는다.
- 설정의 `로그인 세션 삭제`는 프로필, 보관 쿠키와 플랫폼 사용 설정을 모두 지운다. `사용 중지`는 보관된
...
모바일 터치 포커스, 화면 크기 변경, 입력 재시도와 창 선택을 확인한다. 실제 계정이나 외부 사이트를 사용하지 않는다.
